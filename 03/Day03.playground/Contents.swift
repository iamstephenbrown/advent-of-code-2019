...
class Grid {
    func minimumIntersectionDistance(routeOne: [String], routeTwo: [String]) -> Int? {
        // build movements
        guard let firstMovements = try? buildMovements(route: routeOne), let secondMovements = try? buildMovements(route: routeTwo) else {
            return nil
        }
        
        // get the points that each wire travels through
        let centralPoint = Point(x: 0, y: 0)
        let routeOnePoints = buildPoints(movements: firstMovements, from: centralPoint)
        let routeTwoPoints = buildPoints(movements: secondMovements, from: centralPoint)
...
func testPartOne(routeOne: [String], routeTwo: [String], expected: Int) -> String {
    let grid = Grid()
    guard let intersectionDistance = grid.minimumIntersectionDistance(routeOne: routeOne, routeTwo: routeTwo) else {
        return "Fail"
    }
...
/// Answer
let result = Grid().minimumIntersectionDistance(
